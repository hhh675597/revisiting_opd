...
def compute_opd_advantage(
        ...
        reward_weight: float = 0.0,
        multi_turn: bool = True,
        tokenizer=None,   # 传入学生 tokenizer，用于识别 <|im_end|>
):
    """
    Compute advantage for on-policy distillation (OPD), with optional masking of <|im_end|> token
    for 'math' tasks to avoid spurious KL penalty due to tokenizer/template mismatch.

    Args:
        data: DataProto object containing batch, non_tensor_batch, and meta_info
        gamma: discount factor for seq-level OPD (0.0 for token-level)
        reward_weight: weight for outcome reward
        multi_turn: whether the data is multi-turn dialogue
        tokenizer: tokenizer object for converting <|im_end|> to token_id

    Returns:
        advantages: (bs, response_length) tensor
        returns: (bs, response_length) tensor (same as advantages)
    """
    responses = data.batch["responses"]
    response_length = responses.size(1)
    batch_size = data.batch.batch_size[0]
    ...
    if len(unique_tasks) != 1:
        raise ValueError(f"Mixed task batch detected: {unique_tasks}")
    ...
    # --- mask <|im_end|> token for math task ---
    if tokenizer is not None and task_name == "math":
        ignore_token_id = tokenizer.convert_tokens_to_ids("<|im_end|>")
        ignore_mask = (responses != ignore_token_id).float()
        kl_mask = response_mask * ignore_mask
        ...
        #     f.write(
        #         f"task={task_name}, "
        #         f"ignore_token_id={ignore_token_id}, "
        #         f"num_<|im_end|>={num_im_end}, "
        #         f"response_mask_ones={response_mask_ones}, "
        #         f"kl_mask_ones={kl_mask_ones}\n"
        #     )
    else:
    ...
    # --- discounted KL for seq-level OPD ---
    if gamma > 0.0:
        with torch.no_grad():
            discounted_kl = torch.zeros_like(kl_divergence)
            running_discounted_kl = torch.zeros(batch_size, device=kl_divergence.device)
            for t in reversed(range(response_length)):
                running_discounted_kl = kl_divergence[:, t] + gamma * running_discounted_kl
                discounted_kl[:, t] = running_discounted_kl
                running_discounted_kl = running_discounted_kl * kl_mask[:, t]  # reset after EOS
            kl_divergence = discounted_kl
    ...
    advantages = token_level_rewards * kl_mask  # 使用新的 mask
    returns = advantages
    return advantages, returns
